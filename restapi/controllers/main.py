...
class RestApi(http.Controller):
    ...
    def call_report(self, xml_id, id=None, **kwargs):
        ...
        try:
            ...
            return self.get_response(403, '403', json.dumps({'error': {'code': 403, 'message': str(e)}}))
        if datas and not len(datas)>1:
            datas = datas[0]
        return self.get_response(200, str(200), json.dumps({'report': datas}))
    ...
    def perform_multi_request(self, object, id=None, method=None, **kwargs):
        auth, user, invalid = self.valid_authentication(kwargs)
        if not user or invalid:
            return self.get_response(401, str(401   ), json.dumps({'code': 401, 'message': 'Authentication required'}))
    ...
    def perform_request(self, object, method=None, id=None, kwargs={}, user=None):
        ...
        try:
            ...
            if data and isinstance(data, list):
                ...
                return self.get_response(404, '404', json.dumps({'code': 404, 'message': 'Record not found'}))

        except requests.exceptions.RequestException:
            return self.get_response(403, '403', json.dumps({'code': 403, 'message': "Connection Error"}))
        except Exception as e:
            return self.get_response(403, '403', json.dumps({'error': {'code': 403, 'message': str(hasattr(e, 'message') and str(e.message) or e)}}))
